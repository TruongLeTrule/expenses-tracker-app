		...
		if (allExpenses && Array.isArray(allExpenses)) {
			allExpenses.map((expense) => {
				if (
					...
					expense?.date?.seconds * 1000 >= start * 1000 &&
					expense?.date?.seconds * 1000 <= end * 1000
				) {
			...
			<View style={styles.container}>
				<FlatList
					...
					renderItem={({ item, index }) => {
						const budgetsForTimeRange = data.filter(
							(budget) => budget?.timerange?.type === item,
						);
											...
											<Budgets
												...
												value={commafy(
													totalBudget(
														budget.value,
														budget.category,
														budget?.timerange?.start.seconds,
														budget?.timerange?.end.seconds,
													),
												)}
					...
					ListFooterComponent={
						...
						data.length !== 0 && (
							<View className="items-center mt-8">
								<AddButton
									onPress={() => {
										useStore.setState({ modalVisible: true }),
