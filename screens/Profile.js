...
export default function Profile({ navigation }) {
  ...
  const handleTextChange = (type, text) => {
    ...
    if (type === 'firstname' || type === 'lastname') {
      newUserInfo = { ...tempUserInfo, name: { ...tempUserInfo.name, [type]: text } }
      setTempUserInfo(newUserInfo)
      return
    }
    if (type === 'number') {
      newUserInfo = { ...tempUserInfo, address: { ...tempUserInfo.address, [type]: (isNaN(parseInt(text)) ? '' : parseInt(text)) } }
      setTempUserInfo(newUserInfo)
      return
    }
    if (type === 'street' || type === 'city') {
      newUserInfo = { ...tempUserInfo, address: { ...tempUserInfo.address, [type]: text } }
      setTempUserInfo(newUserInfo)
    }
    else {
    ...
    navigation.setOptions(
      {
        headerRight: () => (
          <TouchableOpacity onPress={() => handleEditProfile()}>
            <Text style={{ marginRight: 20, fontSize: 17, fontWeight: 500, color: 'blue' }}>SAVE</Text>
          </TouchableOpacity>
        )
      }
    )
